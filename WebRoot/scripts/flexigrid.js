...
(function ($) {
	$.addFlex = function (t, p) {
		...
		p = $.extend({ //apply default properties
			...
			errormsg: {'chinese':'请求超时','english':'Connection Error'}, //错误提示信息    
			usepager: false,//是否分页
			nowrap: true, //是否不换行
			page: 1, //默认当前页
			total: 0, //总页面数
			...
			title: false,//是否包含标题
			idProperty: 'id',
			pagestat: {'chinese':'当前记录从 {from}  到 {to}，共 {total} 条','english':'Displaying {from} to {to} of {total} items'}, //显示当前页和总页面的样式    
			pagetext: {'chinese':'当前','english':'Page'},
			outof: {'chinese':'共','english':'of'},
			findtext: {'chinese':'搜索','english':'Find'},
			params: [], //allow optional parameters to be passed around
			procmsg: {'chinese':'正在查询, 请稍后 ...','english':'Processing, please wait ...'}, //正在处理的提示信息   
                        showTableBtn : {'chinese':'显示/隐藏 表格','english':'Minimize/Maximize Table'}, //显示隐藏table的提示信息
                        showColumnBtn : {'chinese':'显示/隐藏 列','english':'Hide/Show Columns'}, //显示隐藏column的提示信息
			query: '', //搜索查询的条件    
			qtype: '', //搜索查询的类别    
			nomsg: {'chinese':'没有搜索到结果','english':'No items'}, //无结果的提示信息
                        tdOnpress: false,
			minColToggle: 1, //minimum allowed column to be hidden    
			showToggleBtn: true, //show or hide column toggle popup
			hideOnSubmit: true, //回调时显示遮盖    
			autoload: true, //自动加载    
...
//                                                var colArr = [];
//                                                $.each($(this.dcol).parent().find("div"),function(i,o){
//                                                    colArr.push($(o).parent().attr("abbr"));
//                                                });
//                                                if ($.cookie) {
//                                                    $.cookie('flexiMed/fieldSort',"["+colArr.join(",")+"]" ,{expires:3});
//                                                }
					}
					this.dcol = null;
					this.hset = null;
			...
			addData: function (data) { //parse data
				...
				this.loading = false;
				if (!data) {
					$('.pPageStat', this.pDiv).html(p.errormsg[p.language]);
					return false;
				}
				if (p.dataType == 'xml') {
				...
				if (p.total == 0) {
					...
					p.page = 1;
					this.buildpager();
					$('.pPageStat', this.pDiv).html(p.nomsg[p.language]);
					return false;
				}
							...
							function () {
								...
								if (typeof row.cell == 'undefined') {
									...
									if (typeof row.cell[idx] != "undefined") {
										td.innerHTML = (row.cell[idx] != null) ? unescape(row.cell[idx]) : '';//null-check for Opera-browser
									} else {
			...
			buildpager: function () { //rebuild pager based on new properties
				...
				var stat = p.pagestat[p.language];
			...
			populate: function () { //get latest data
				...
				$('.pPageStat', this.pDiv).html(p.procmsg[p.language]);
				...
				$('tbody tr', g.bDiv).on('click', function (e) {
					...
					if (p.onDoubleClick) {
						p.onDoubleClick(this, g, p);
                                        ...
                                        var temp_div = e.target || e.srcElement;
                                        if($(temp_div).parent().attr("abbr") == p.tdOnpress.name){
                                            var trId = $(temp_div).parent().parent().attr("id").replace("row","");
                                            var tdId = $(temp_div).html();
                                            var drugname;
                                            $(temp_div).parent().parent().find("td").each(function(){
                                                if($(this).attr("abbr")=="medicineChineseName"){
                                                    drugname = $(this).find("div").text();
                                                }
                                            });
                                            p.tdOnpress.press(trId,tdId,drugname);
                                            return;
                                        }
			...
			for (var i = 0; i < p.colModel.length; i++) {
				...
				if( cm ) {	// only use cm if its defined
					...
					if( cm.display != undefined ) {
						th.innerHTML = cm.display[p.language];
					}
		...
		if (p.buttons) {
			...
			for (var i = 0; i < p.buttons.length; i++) {
				...
				if (!btn.separator) {
					...
					btnDiv.innerHTML = ("<div><span>") + (btn.hidename ? "&nbsp;" : btn.name[p.language]) + ("</span></div>");
					...
					btnDiv.name = btn.name[p.language];
					if (btn.id) {
						btnDiv.id = btn.id;
					}
					if (btn.onpress) {
		...
		$(g.hDiv).append('<div class="hDivBox" onselectstart="return false;"></div>');
		...
		if (p.usepager) {
			...
			$(g.bDiv).after(g.pDiv);
			var html = ' <div class="pGroup"> <div class="pFirst pButton"><span></span></div><div class="pPrev pButton"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"><span class="pcontrol">' + p.pagetext[p.language] + ' <input type="text" size="4" value="1" /> ' + p.outof[p.language] + ' <span> 1 </span></span></div> <div class="btnseparator"></div> <div class="pGroup"> <div class="pNext pButton"><span></span></div><div class="pLast pButton"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"> <div class="pReload pButton"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"><span class="pPageStat"></span></div>';
			...
			if (p.searchitems) {
				...
				for (var s = 0; s < sitems.length; s++) {
					...
					sopt += "<option value='" + sitems[s].name + "' " + sel + " >" + sitems[s].display[p.language] + "&nbsp;&nbsp;</option>";
				}
				if (p.qtype == '') {
					p.qtype = sitems[0].name;
				}
                                //添加搜索条件的规则
                                var qops = "<input type='radio' name='qop' value='='>= <input type='radio' name='qop' value='>' />><input type='radio' name='qop' value='<'><";
                                qops += "<input type='radio' name='qop' value='>='>>=<input type='radio' name='qop' value='<=' /><=<input type='radio' name='qop' value='like' checked='checked'>like";
				$(g.sDiv).append("<div class='sDiv2'>" + p.findtext[p.language] +
						" <input type='text' value='" + p.query +"' size='30' name='q' class='qsbox' /> "+
						" <select name='qtype'>" + sopt + "</select> "+ qops +"</div>");
				...
				$('input[value=Clear]', g.sDiv).click(function () {
					...
					g.doSearch();
                                ...
                                $('input[name=qop]', g.sDiv).click(function(e){
                                    p.qop = $(this).val();
                                    g.doSearch();
                                });
		...
		if (p.title) {
			...
			g.mDiv.innerHTML = '<div class="ftitle">' + p.title[p.language] + '</div>';
			$(g.gDiv).prepend(g.mDiv);
			if (p.showTableToggleBtn) {
				$(g.mDiv).append('<div class="ptogtitle" title="' + p.showTableBtn[p.language] + '"><span></span></div>');
		...
		if ($('th', g.hDiv).length) {
			...
			$('input.togCol', g.nDiv).click(function () {
				if ($('input:checked', g.nDiv).length < p.minColToggle && this.checked == false) return false;
                                    ...
                                    if($.cookie) {
                                        var showArr = [];
                                        $.each($('input[type=checkbox]',g.nDiv) ,function(i,o) {
                                            if(o.checked) {
                                                showArr.push(false);
                                            } else {
                                                showArr.push(true);
                                        ...
                                        $.cookie('flexiMed/ColShow', "["+showArr.join(",")+"]",{expires:3});
                                    }
			});
			$(g.gDiv).prepend(g.nDiv);
			$(g.nBtn).addClass('nBtn')
				.html('<div></div>')
				.attr('title', p.showColumnBtn[p.language])
				.click(function () {
					$(g.nDiv).toggle();
					return true;
				}
